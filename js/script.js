...
function loadMain(file_json) {
  ...
  for (let room in file_json) {
    ...
    for (let item in file_json[room]['Item']) {
      ...
      resp += '"><div class="item-info"><div class="item-key"><div>Código</div><div class="larger">Item</div><div>Status</div><div>Sugestão</div><div>Disponib.</div></div><div class="item-value"><div>'
      ...
      resp += '</div><div class="item-note"><div class="item-key"><div>Quantidade</div><div>Altura</div><div>Largura</div><div>Profund.</div><div>Avaria</div></div><div class="item-value"><div>'
